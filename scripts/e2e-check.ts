/**
 * End-to-end smoke check for midnight-ballot.
 *
 * Reconnects to the deployed contract, reads its ledger state, and exits 0
 * on success. Used by `npm run test:e2e` and the CI workflow.
 *
 * Requires: deployed contract + running proof server + devnet (or public network).
 */
...
const PRIVATE_STATE_ID = 'ballotPrivateState';
...
async function main() {
  ...
  if (!deployment) {
    console.error(`No deploy on file for network ${network}. Run: npm run deploy`);
    process.exit(1);
  }
  ...
  const zkConfigPath = path.resolve(__dirname, '..', 'contracts', 'managed', 'ballot');
  ...
  const providers = {
    privateStateProvider: levelPrivateStateProvider({
      privateStateStoreName: 'ballot-state',
